...
# TODO: Need to write the MRL model adapter here. Basically you have the train_lightning.py using the custom multinerf model
#       from the models folder. The MRL adapter should basically be able to be use the MRL model logic in the train_lightning.py in place 
#       of the MultiNeRF model. This will allow us to use the MRL model in the train_lightning.py script. 
#       It would probably be a good idea to make a train_lightning2.py that mostly uses the same code as train_lightning.py but with the MRL model.
#       The adapter will basically make sure the data passed to the MRL model and the data returned is in the correct expected format. See multimodel.py for the formats. 
#       most of the code written will be here, with minor changes in train_lightning2.py to use the MRL model.
